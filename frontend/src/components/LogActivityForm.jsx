    ...
    <form onSubmit={handleSubmit} className="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-7 shadow-sm transition-all relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 mb-5">
        <div className="flex flex-col gap-1.5 relative" ref={dropdownRef}>
          <label htmlFor="activity-type-btn" className="text-xs font-semibold uppercase tracking-wider text-slate-700">
            Activity Type
          </label>
          <button
            id="activity-type-btn"
            type="button"
            aria-haspopup="listbox"
            aria-expanded={dropdownOpen}
            aria-label={`Activity Type: ${selected?.label || type}`}
            onClick={() => setDropdownOpen(!dropdownOpen)}
            className="w-full h-10 px-3.5 bg-slate-50/50 hover:bg-white focus:bg-white border border-slate-200 rounded-xl text-sm text-slate-800 flex items-center justify-between transition-all font-medium cursor-pointer shadow-sm text-left focus:outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10"
          >
          ...
          {dropdownOpen && (
            <div
              ...
              className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl p-1.5 z-50 flex flex-col gap-0.5 animate-in fade-in zoom-in-95 duration-100 min-w-[320px] max-w-[380px] w-max"
            >
              {Object.entries(activityTypes).map(([key, def]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-mono text-[11px] text-slate-500 bg-slate-50 border border-slate-200/80 px-2 py-0.5 rounded font-normal shrink-0">
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="activity-quantity" className="text-xs font-semibold uppercase tracking-wider text-slate-700">
            Quantity {selected ? <span className="font-mono text-[11px] font-normal text-slate-500 lowercase">({selected.unit})</span> : ""}
          </label>
          <input
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="activity-date" className="text-xs font-semibold uppercase tracking-wider text-slate-700">
            Date
          </label>
          <input
            id="activity-date"
